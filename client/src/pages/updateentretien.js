...
const EditEntretien = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setMessage("Entretien modifié avec succés");
      ...
      if (originalForm.heure !== form.heure) {
        // If the date has changed, send a notification with the new date
        let message = `Il y a une modification dans l'heure l'entretien de l'offre ${nomOffre} La nouvelle heure est : ${form.heure}`;
        originalForm.user.forEach(userId => {
          notificationPromises.push(
            axios.post('/api/notification', {
              message,
      ...
      if (originalForm.type !== form.type) {
        ...
        let message = `Il y a une modification dans la date l'entretien de l'offre ${nomOffre} Votre entretien est devenu ${form.type}.`;
